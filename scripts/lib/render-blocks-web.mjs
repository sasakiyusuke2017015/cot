 ...
 *
 * `</` を `<\/` にするのは必須。この文字列は最終的に <script type="text/babel"> の
 * 中に置かれるが、HTML パーサは JS の文字列の内側かどうかに関係なく `</script>` を見た
 * 時点でスクリプトを閉じてしまう。教材はコード例に `</script>` を普通に含むため、
 * エスケープしないとその章のページが丸ごと動かなくなる（実際に第2章・第3章が壊れた）。
 * テンプレートリテラル内の `\/` は `/` として解釈されるので、表示は元のままになる。
 */
