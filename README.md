...
**push しても自動では公開されない。** 公開したいときだけ手動で実行する。

```bash
gh workflow run pages.yml --repo sasakiyusuke2017015/cot
```

（GitHub の Actions 画面から「Run workflow」でも可）

> **⚠️ GitHub Pages は 1時間あたり10回までしかデプロイできない。**
> 自動デプロイにしていると、まとめて作業した日に制限へ当たり、
> **どの回も反映されなくなる**（実際に14回連続で失敗した）。
> 失敗しても連続で再実行せず、**1時間空けてから1回だけ**試すこと。
>
> また `deploy-pages` はデプロイ成功後の完了検知でタイムアウトすることがある。
